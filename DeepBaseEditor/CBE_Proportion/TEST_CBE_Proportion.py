...
## System Paths ##
path                 = './'
parameters           = {'0': 'CBE_Proportion_sample.txt'} # Dictionary can be expanded for multiple test parameters
...
class Deep_xCas9(object):
    def __init__(self, filter_size, filter_num, node_1 = 80, node_2 = 60, l_rate = 0.005, window_size = 5):
        ...
        with tf.variable_scope('Output_Layer'):
            W_out        = tf.get_variable("W_out", shape=[node_2, 2**window_size-1])#, initializer=tf.contrib.layers.xavier_initializer())
...
def req_seq_produce(seq):
    ...
    for indiv_seq in seq:
        ...
        for index in range(window_size):
            if indiv_seq[window_start+index] == 'C':
                print(index, indiv_seq[window_start:window_start+index])
                tmp = []
                for tmp_indiv_seq in tmp_seq:
                    tmp.append(tmp_indiv_seq[:window_start+index]+str("T")+tmp_indiv_seq[window_start+index+1:])
