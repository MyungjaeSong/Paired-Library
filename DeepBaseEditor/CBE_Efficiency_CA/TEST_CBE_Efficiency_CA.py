...
## System Paths ##
path                 = './'
parameters           = {'0': 'cbe_wt_ca_sample.txt'} # Dictionary can be expanded for multiple test parameters

## Run Parameters ##
TEST_NUM_SET         = [0] # List can be expanded in case of multiple test parameters
best_model_path_list = ['./cbe_wt_ca_best']
...
class Deep_xCas9(object):
    def __init__(self, filter_size, filter_num, node_1 = 80, node_2 = 60, l_rate = 0.005):
        ...
        with tf.variable_scope('CA_Layer'):
            W_acc = tf.get_variable("W_acc", shape=[1, node_2])#, initializer=tf.contrib.layers.xavier_initializer())
            B_acc = tf.get_variable("B_acc", shape=[node_2])#, initializer=tf.contrib.layers.xavier_initializer())
            L_ca_pre = tf.nn.bias_add(tf.matmul(self.ca, W_acc), B_acc)
            
        L_fcl2_drop = tf.multiply(L_fcl2_drop, L_ca_pre)
        ...
        self.optimizer = tf.train.AdamOptimizer(l_rate).minimize(self.obj_loss, 
                                                                var_list= (tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='CA_Layer')
                                                                          +tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='Output_Layer')))
    #def end: def __init__
#class end: Deep_xCas9

# Test Model
def Model_Inference(sess, TEST_X, TEST_CA, model, args, load_episode, test_data_num, testvalbook, testvalsheet, col_index=1):
    test_batch = 500
    test_spearman = 0.0
    ...
    for i in range(int(ceil(float(TEST_X.shape[0])/float(test_batch)))):
        Dict = {model.inputs: TEST_X[i*test_batch:(i+1)*test_batch], model.ca: TEST_CA[i*test_batch:(i+1)*test_batch], model.is_training: False}
...
def getfile_inference(filenum):
    ...
    ca = expand_dims(array(ca), 1)

    return processed_full_seq, seq, ca
#def end: getseq
...
for TEST_NUM_index in range(len(TEST_NUM_SET)):
    ...
    tmp_X, pre_X, tmp_CA = getfile_inference(TEST_NUM)
    TEST_X.append(tmp_X)
    TEST_CA.append(tmp_CA)
    test_row = 0
    for index_X in range(np.shape(pre_X)[0]):
        testsheet[-1][-1].write(test_row, 0, pre_X[index_X])
        testsheet[-1][-1].write(test_row, 1, tmp_CA[index_X][0])
        test_row += 1
...
for index in range(len(best_model_list)):
    ...
    with tf.Session(config=conf) as sess:
        ...
        for i in range(len(TEST_NUM_SET)):
            Model_Inference(sess, TEST_X[i], TEST_CA[i], model, args, load_episode, TEST_NUM_SET[i], testbook, testsheet[i])
        testbook.close()
